...
export async function getPullRequestCommit(
...
export async function approve(
  ...
  try {
    ...
    let commit = await getPullRequestCommit(client, context, prNumber);

    while (
      maxRetries-- > 0 &&
      commit.substring(0, commitSha.length) != commitSha
    ) {
      commit = await getPullRequestCommit(client, context, prNumber);
    }
